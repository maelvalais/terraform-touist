...
resource "aws_launch_configuration" "ecs_instance" {
  ...
  security_groups             = ["${split(",", var.security_group_ecs_instance_ids)}"]
  image_id                    = "${var.ami}"
...
# A dummy resource I created so that the aws_autoscaling_group (ASG) is
# created AFTER the aws_nat_gateway (ANG).
# https://github.com/terraform-aws-modules/terraform-aws-vpc/issues/96
resource "null_resource" "bootstrap" {
  provisioner "local-exec" {
    command = "echo 'Dummy echo command so that we wait for aws_nat_gateway (id: ${var.nat_gateway_ids}) to be done before we launch aws_autoscaling_group'"
  }
}
